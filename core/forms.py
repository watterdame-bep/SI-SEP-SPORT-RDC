...
from gouvernance.models import ProvAdmin, DisciplineSport
...
# --- Discipline sportive (utilisé ailleurs, ex. admin) ---

_STYLE_INPUT = 'w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 placeholder-slate-400 focus:border-rdc-blue focus:ring-1 focus:ring-rdc-blue/30 focus:outline-none transition'


class DisciplineSportForm(forms.ModelForm):
    """Formulaire discipline sportive (utilisé ailleurs, ex. admin)."""
    class Meta:
        model = DisciplineSport
        fields = ('code', 'designation', 'ordre', 'actif')
        widgets = {
            'code': forms.TextInput(attrs={'class': _STYLE_INPUT, 'placeholder': 'Ex. FOOTBALL'}),
            'designation': forms.TextInput(attrs={'class': _STYLE_INPUT, 'placeholder': 'Ex. Football'}),
            'ordre': forms.NumberInput(attrs={'class': _STYLE_INPUT, 'min': 0}),
        }
